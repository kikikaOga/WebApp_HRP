...
if st.button("🎯 ทำนายความเสี่ยง" , use_container_width=True, type="primary"):
    if model is None:
        ...
        try:
            ...
            st.metric(label="ระดับความเสี่ยง", value=risk_level, delta=f"{confidence:.1f}% confidence")
            st.write(f"- โอกาส Low Risk (0): {probability[0]*100:.1f}%")
            st.write(f"- โอกาส High Risk (1): {probability[1]*100:.1f}%")
